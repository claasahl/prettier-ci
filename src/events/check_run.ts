...
function created2ChecksUpdateParams(
  params: Partial<gh.ChecksUpdateParams>
): Projection<gh.ChecksUpdateParams> {
  return context => {
...
function created2ReposCompareCommitsParams(
  context: Context
): gh.ReposCompareCommitsParams {
  const owner = context.payload.repository.owner.login;
  const repo = context.payload.repository.name;
...
function created2ReposGetContent(
  params: Overwrite<Partial<gh.ReposGetContentParams>, { path: string }>
): Projection<gh.ReposGetContentParams> {
  return context => {
    const owner = context.payload.repository.owner.login;
    const repo = context.payload.repository.name;
...
async function markCheckAsInProgress(context: Context): Promise<Context> {
  const result = await checks.update(
    context,
    created2ChecksUpdateParams({ status: "in_progress" })
  );
...
async function fetchModifiedFiles(
  context: Context
): Promise<{ context: Context; files: string[]; ref: string }> {
  const commits = await repos.compareCommits(
    context,
    created2ReposCompareCommitsParams
  );
  const { head } = created2ReposCompareCommitsParams(context);
  const files: string[] = commits.response.data.files
    .filter((file: any) => file.status !== "deleted")
...
async function fetchContent(data: {
  ...
  file: string;
  sha: string;
}): Promise<{ context: Context; file: string; content: string; sha: string }> {
  const result = await repos.getContent(
    data.context,
    created2ReposGetContent({ path: data.file, ref: data.sha })
  );
  if (result.response.data.encoding === "base64") {
    ...
    return {
      ...
      file: data.file,
      content,
      sha: result.response.data.sha
    };
  } else {
    throw new Error(
      "unsupported encoding '" +
        result.response.data.encoding +
        "' for file '" +
        data.file +
        "'"
    );
...
async function checkContent(data: {
  ...
  file: string;
  content: string;
}): Promise<{ context: Context; file: string; passed: boolean }> {
  const passed = await prettier.check(data.content, { filepath: data.file });
  return { context: data.context, file: data.file, passed };
}

async function formatContent(data: {
  context: Context;
  file: string;
  content: string;
}): Promise<{ context: Context; file: string; content: string }> {
  const formatted = await prettier.format(data.content, {
    filepath: data.file
  });
  return { context: data.context, file: data.file, content: formatted };
}

async function markCheckAsCompleted(data: {
  context: Context;
  report: Partial<gh.ChecksUpdateParams>;
}): Promise<Context> {
  const completed_at = new Date().toISOString();
  const result = await checks.update(
    data.context,
    created2ChecksUpdateParams({
      ...data.report,
      status: "completed",
      completed_at
    })
...
function rerequested2ChecksCreateParams(
  context: Context
): gh.ChecksCreateParams {
  const owner = context.payload.repository.owner.login;
  const repo = context.payload.repository.name;
...
function asReport(results: FileCheck[]): Partial<gh.ChecksUpdateParams> {
  ...
  return {
    output: { title: "Prettier", summary, text },
...
async function requested_action_fix(context: Context): Promise<Context> {
  const createReference = await gitdata.createReference(
    context,
    fix2CreateReferenceParams
  );
  ...
  for (const file of files) {
    ...
    if (!passed) {
      ...
      await repos.updateFile(
        context,
        fix2ReposUpdateFileParams({
          path: file,
          content: btoa(formatted.content),
          branch,
          sha
        })
      );
...
function fix2CreateReferenceParams(
  context: Context
): gh.GitdataCreateReferenceParams {
  const owner = context.payload.repository.owner.login;
  const repo = context.payload.repository.name;
  const ref =
    REFERENCE_PREFIX + context.payload.check_run.check_suite.head_branch;
  const sha = context.payload.check_run.check_suite.head_branch;
  return { owner, repo, ref, sha };
}

function fix2ReposUpdateFileParams(
  params: Overwrite<
    Partial<gh.ReposUpdateFileParams>,
    { path: string; content: string; branch: string; sha: string }
  >
): Projection<gh.ReposUpdateFileParams> {
  return context => {
    const owner = context.payload.repository.owner.login;
    const repo = context.payload.repository.name;
...
function fix2PullRequestsCreateParams(
  params: Overwrite<
    Partial<gh.PullRequestsCreateParams>,
    { head: string; body: string }
  >
): Projection<gh.PullRequestsCreateParams> {
  return context => {
    const owner = context.payload.repository.owner.login;
    const repo = context.payload.repository.name;
...
function asPullRequestBody(context: Context, results: FileCheck[]): string {
  const failedResults = results.filter(result => !result.passed);
  return `This [check run](${context.payload.check_run.html_url}) identified ${
    failedResults.length
  } ${failedResults.length == 1 ? "file" : "files"}, which ${
    failedResults.length == 1 ? "needs" : "need"
  } formatting.

...
export async function created(context: Context): Promise<void> {
  ...
  for (const file of files) {
    ...
    const { passed } = await checkContent({
      context,
      file: file.filename,
      content
    });
...
async function fetchModifiedFiles(
  context: Context
): Promise<{ context: Context; files: { filename: string; sha: string }[] }> {
  ...
  const files: {
    filename: string;
    sha: string;
  }[] = commits.response.data.files
    .filter((file: any) => file.status !== "deleted")
    .map((file: any) => ({ filename: file.filename, sha: file.sha }));

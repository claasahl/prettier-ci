 ...
 */
export async function checkFiles (params: CheckFilesParams): Promise<CheckFilesResult> {
 ...
 */
export async function checkFile (params: CheckFileParams): Promise<CheckFileResult> {
  const { context, owner, repo, sha, file } = params
  const prettierOptions = { filepath: file }
  const errors: Error[] = []

  try {
    const reposGetContentParams: ReposGetContentParams = { owner, repo, path: file, ref: sha }
    const contentResponse: Response<any> = await context.github.repos.getContent(reposGetContentParams)
  ...
  return {
    owner, repo, sha, file, passed: errors.length === 0, errors
  }
}

...
export = (app: Application) => {
  ...
  app.on(`*`, async context => {
    ...
    return Rx.race(
      ...
      event.pipe(
        ofEvent("check_suite.requested"),
        flatMap(check_suite.requested)
      ),
      event.pipe(
        ofEvent("check_run.rerequested"),
        flatMap(check_run.rerequested)
      ),
      event.pipe(
        ofEvent("check_run.created"),
        flatMap(check_run.created)
      )
    )
      .pipe(
        retry(2),

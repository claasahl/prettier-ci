...
describe("tests for file-analysis (check_run)", () => {
  ...
  test("retrieves contents for file-version for check_run", async () => {
    ...
    const params: gh.ReposGetContentParams = {
      ...
      ref: "52c67357a6b1858226c131d13d7d44e8303fd426"
    };
  ...
  test("completes file-analysis with correct conclusion", async () => {
    ...
    const params: gh.ChecksUpdateParams = {
      ...
      conclusion: "success",
      output: {
        summary: "Pretty. Keep up the **good work**.",
        title: "Prettier",
        text: undefined
      },
      actions: [],
...
describe("test for pull request (fix of errors in check_run)", () => {
    ...
    github = {
      repos: {
        compareCommits: jest
          .fn()
          .mockImplementation(
            mockReposCompareCommits("./test/templates/unformatted.json")
          ),
  ...
  test("'fix' action should create branch", async () => {
    ...
    const params: gh.GitdataCreateReferenceParams = {
      owner: "claasahl",
      repo: "prettiest-bot",
      ref: "refs/heads/prettier/develop",
      sha: "develop"
    };
  ...
  test("compares before/after commits for relevant check_run", async () => {
    ...
    const params: gh.ReposCompareCommitsParams = {
      owner: "claasahl",
      repo: "prettiest-bot",
      base: "a137828f79b51c516391b0d125b8df492fdfb3ce",
      head: "bb4d2ed9702c4c4340db50f74fc451657fe48e57"
    };
    expect(github.repos.compareCommits).toHaveBeenCalledWith(params);
  });
  ...
  test("retrieves contents for file-version for relevant check_run", async () => {
    ...
    const params: gh.ReposGetContentParams = {
      owner: "claasahl",
      repo: "prettiest-bot",
      path: "./test/templates/unformatted.json",
      ref: "refs/heads/prettier/develop"
    };
    expect(github.repos.getContent).toHaveBeenCalledWith(params);
  });
  ...
  test("create pull request for fixed file (singular)", async () => {
    github.repos.getContent = jest
      .fn()
      .mockImplementation(mockReposGetContentUnformatted);
    await app.receive(checkRunRequestedActionEvent);

    const body = template(
      "https://github.com/claasahl/prettiest-bot/runs/16344324",
      1,
      "claasahl"
    );
    const params: gh.PullRequestsCreateParams = {
      owner: "claasahl",
      repo: "prettiest-bot",
      base: "develop",
      head: "refs/heads/prettier/develop",
      title: "Prettified branch: develop",
  ...
  test("create pull request for fixed files (plural)", async () => {
    github.repos.compareCommits = jest
      .fn()
      .mockImplementation(
        mockReposCompareCommits(
  ...
  test("format and update files that need fixing", async () => {
    ...
    let content = btoa(
      prettier.format(
        fs.readFileSync("./test/templates/unformatted.ts").toString(),
        { filepath: "./test/templates/unformatted.ts" }
      )
    );
    let params: gh.ReposUpdateFileParams = {
      owner: "claasahl",
      repo: "prettiest-bot",
      message: "formatted file: ./test/templates/unformatted.ts",
      path: "./test/templates/unformatted.ts",
      branch: "refs/heads/prettier/develop",
      sha: "7d6e8ee3e4c26b6d5d305fa3fe985ddf7c0f87ea",
      content
    };
    ...
    content = btoa(
      prettier.format(
        fs.readFileSync("./test/templates/unformatted.json").toString(),
        { filepath: "./test/templates/unformatted.json" }
      )
    );
    params = {
      owner: "claasahl",
      repo: "prettiest-bot",
      message: "formatted file: ./test/templates/unformatted.json",
      path: "./test/templates/unformatted.json",
      branch: "refs/heads/prettier/develop",
      sha: "7d6e8ee3e4c26b6d5d305fa3fe985ddf7c0f87ea",
      content
  ...
  function mockReposCompareCommits(...files: string[]) {
    return async () => {
      ...
      base.data.files = files.map(file => ({
        filename: file,
        status: "modified"
      }));
      return base;
  ...
  async function mockReposGetContentFormatted(
    params: gh.ReposGetContentParams
  ) {
  ...
  async function mockReposGetContentUnformatted(
    params: gh.ReposGetContentParams
  ) {
    const base = reposgetContent_200;
    const unformatted = fs.readFileSync(params.path).toString();

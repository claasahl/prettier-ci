...
describe("test for pull request (fix of errors in check_run)", () => {
  ...
  test('retrieves contents for file-version for relevant check_run', async () => {
    ...
    const params: gh.ReposGetContentParams = { owner: "claasahl", repo: "prettiest-bot", path: "./test/templates/unformatted.json", ref: "refs/heads/prettier/develop"};
    expect(github.repos.getContent).toHaveBeenCalledWith(params)
  })
  ...
  test('create pull request for fixed files (plural)', async () => {
    ...
    const body = template("https://github.com/claasahl/prettiest-bot/runs/16344324", 2, "claasahl")
    const params: gh.PullRequestsCreateParams = { owner: "claasahl", repo: "prettiest-bot", base: "develop", head: "refs/heads/prettier/develop", title: "Prettified branch: develop", maintainer_can_modify: true, body};
    expect(github.pullRequests.create).toHaveBeenCalledWith(params)
  })
  ...
  test('format and update files that need fixing', async () => {
    ...
    let params: gh.ReposUpdateFileParams = { owner: "claasahl", repo: "prettiest-bot", message: "formatted file: ./test/templates/unformatted.ts", path: "./test/templates/unformatted.ts", branch: "refs/heads/prettier/develop", sha: "7d6e8ee3e4c26b6d5d305fa3fe985ddf7c0f87ea", content};

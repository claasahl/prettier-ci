...
describe("tests for check_run parameters", async () => {
    ...
    test("parameters for 'checks.update' [success]", async () => {
        ...
        mockdate.reset()
        expect(params).toEqual({status:"completed", conclusion: "success", completed_at})
    })
    test("parameters for 'checks.update' [failure]", async () => {
        ...
        mockdate.reset()
        expect(params).toEqual({status:"completed", conclusion: "failure", completed_at})
    })
    test("parameters for 'checks.update' [cancelled]", async () => {
        ...
        mockdate.reset()
        expect(params).toEqual({status:"completed", conclusion: "cancelled", completed_at})
    })
})